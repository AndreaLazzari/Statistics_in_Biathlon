...
years_men_path <- list.dirs(path = 'biathlon_data/men',
                            recursive=FALSE,full.names=TRUE)

# Path of the directory for each year for women
years_women_path <- list.dirs(path='biathlon_data/women',
                              recursive=FALSE,full.names=TRUE)

# List of years directory names for men
years_men <- list.dirs(path = 'biathlon_data/men', 
                       recursive = FALSE, full.names=FALSE)

# List of years directory names for women
years_women <- list.dirs(path = 'biathlon_data/women', 
                         recursive =FALSE,full.names=FALSE)
...
for (i in seq_along(years)){
  for(j in seq_along(locations[[years[i]]])){
    ...
    for (file in files){
      ...
      # been chosen as a tuple, we need also the name of the runner-up because Samuelsson won back
      ...
      for (file in files){
        ...
        if (
            ...
            for(column in time_columns){
              ...
              # The first case is for where we have minutes and not only second where we have a column 
              # as a separator
              if (grepl(pattern = ':', x  = reference_time,
                            fixed=TRUE)){
...
```{r fig.align="center", fig.width=8 , fig.height=5, warning=FALSE}
